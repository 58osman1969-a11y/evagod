...
FRIENDS = [
    ...
    "SHOURYA","DEAD","AMAN","ERROR","VIO","ARES","VILLAIN","YASH",
...
def _dead_factory(txt: str) -> callable:
    ...
    def _f():
        if _alt[0]:
            c = f"♛𝑫𝑬𝑨𝑫 {txt}{rnd_suffix()}"[:255]
        else:
            c = f"⚜️𝑫𝑬𝑨𝑫 {txt}{rnd_suffix()}"[:255]
        _alt[0] = not _alt[0]
        if c == last[0]:
            _alt[0] = not _alt[0]
            c = (f"♛𝑫𝑬𝑨𝑫 {txt}{rnd_suffix()}" if _alt[0]
                 else f"⚜️𝑫𝑬𝑨𝑫 {txt}{rnd_suffix()}")[:255]
        last[0] = c
        return c
    return _f
...
def _make_elite_cmd(factory_fn, label: str, sym: str):
    ...
    async def _cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await _reply(msg,
            f"╔══════════════════════════════════╗\n"
            f"  {sym*4} ⚔️ 𝑺𝑨𝑺𝑼𝑲𝑬 𝑬𝑳𝑰𝑻𝑬 𝑵𝑪 {sym*4}\n"
            f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"  📛 𝘛𝘦𝘹𝘵:   {txt}\n"
            f"  ⚔️  𝘔𝘰𝘥𝘦:   {label}\n"
...
def _make_legend_cmd(factory_fn, label: str, sym: str):
    ...
    async def _cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await _reply(msg,
            f"╔══════════════════════════════════╗\n"
            f"  {sym*4} ⚡ 𝑺𝑨𝑺𝑼𝑲𝑬 𝑳𝑬𝑮𝑬𝑵𝑫 𝑵𝑪 {sym*4}\n"
            f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"  📛 𝘛𝘦𝘹𝘵:    {txt}\n"
            f"  ⚔️  𝘔𝘰𝘥𝘦:    {label}\n"
            f"  🤖 𝘉𝘰𝘵𝘴:    {len(bots)} (𝘙𝘰𝘶𝘯𝘥-𝘙𝘰𝘣𝘪𝘯)\n"
